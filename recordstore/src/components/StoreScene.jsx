...
import { OrbitControls, Box, Text, Html, Sky, Cylinder } from '@react-three/drei'
...
function VinylTile({ position, color }) {
    return (
        <group position={position}>
            <Box args={[0.8, 0.8, 0.05]} rotation={[Math.PI / 12, 0, 0]}>
                <meshStandardMaterial color={color} />
            </Box>
            <Box args={[0.7, 0.7, 0.01]} position={[0, 0, 0.03]} rotation={[Math.PI / 12, 0, 0]}>
                <meshStandardMaterial color="black" />
            </Box>
            <Box args={[0.2, 0.2, 0.02]} position={[0, 0, 0.04]} rotation={[Math.PI / 12, 0, 0]}>
...
function Shelf({ position, name, color }) {
    ...
    return (
        <group position={position} onClick={(e) => {
            ...
            {/* Vertical supports */}
            <Box args={[0.1, 4, 0.1]} position={[-1.45, 0, 0]}>
            ...
            {[-1.8, -0.6, 0.6, 1.8].map((y, index) => (
                <group key={index}>
                    <Box args={[3, 0.1, 1]} position={[0, y, 0]}>
                        <meshStandardMaterial color="#A0522D" />
                    </Box>
                    {/* Add vinyl tiles to each shelf */}
                    {[-1, 0, 1].map((x, i) => (
            ...
            {/* Name card (visible from both sides) */}
            <group position={[0, 2.7, 0]}>
                <Box args={[1.5, 0.5, 0.05]}>
...
function Speaker({ position }) {
    return (
        <group position={position}>
            ...
            <Cylinder args={[0.3, 0.3, 0.1, 32]} position={[0, 3, 0.5]} rotation={[Math.PI / 2, 0, 0]}>
                <meshStandardMaterial color="#7f8c8d" />
            </Cylinder>
            <Cylinder args={[0.2, 0.2, 0.1, 32]} position={[0, 2, 0.5]} rotation={[Math.PI / 2, 0, 0]}>
                <meshStandardMaterial color="#7f8c8d" />
            </Cylinder>
            <Cylinder args={[0.15, 0.15, 0.1, 32]} position={[0, 1, 0.5]} rotation={[Math.PI / 2, 0, 0]}>
                <meshStandardMaterial color="#7f8c8d" />
            </Cylinder>
        </group>
    )
...
function Room() {
    return (
        <group>
            {/* Floor */}
            <Box args={[50, 0.1, 30]} position={[0, -2, 0]}>
                <meshStandardMaterial color="#708090" />
            </Box>
            {/* Ceiling */}
            <Box args={[50, 0.1, 30]} position={[0, 8, 0]}>
            ...
            {[[-25, 0], [25, Math.PI]].map(([x, rotation], index) => (
                <group key={index} position={[x, 3, 0]} rotation={[0, rotation, 0]}>
                    ...
                    {/* Windows */}
                    {[-10, 0, 10].map((z, i) => (
                        <Box key={i} args={[0.2, 4, 4]} position={[0, 1, z]}>
            ...
            {[-15, 15].map((z, index) => (
                <Box key={index} args={[50, 10, 0.1]} position={[0, 3, z]}>
                    <meshStandardMaterial color="#F5F5F5" />
                </Box>
            ))}
        </group>
...
function Sun() {
    ...
    useFrame(({ clock }) => {
        if (sunRef.current) {
            ...
            sunRef.current.position.x = Math.cos(t) * 20
            sunRef.current.position.y = Math.sin(t) * 20 + 5
        }
    })
    ...
    return (
        <mesh ref={sunRef} position={[20, 5, 0]}>
            ...
            <pointLight color="#FFF" intensity={1} distance={100} />
        </mesh>
    )
}
...
export default function StoreScene() {
    return (
        <div style={{ width: '100vw', height: '100vh' }} className="relative">
            <Canvas camera={{ position: [25, 15, 15], fov: 50 }}>
                <CameraController />
